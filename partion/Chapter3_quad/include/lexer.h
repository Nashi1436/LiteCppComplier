...
private:
    ...
    string keywords[DI_SZ] = { //1-100
        ...
        //2 Type Specifiers
        "void","bool","short","int","long","signed",
        "float","double","decltype","auto",
        ...
        //5 Operators
        "sizeof", "new", "delete", "throw", "noexcept", "typeid",
        ...
        "class", "struct", "union", "enum", "public", "protected", "private",
        //  "friend", "virtual", "explicit", "operator", 
        // "this", "constexpr",
        ...
        //12 Other Keywords"typedef",

        // "using", "namespace", "inline",  "decltype", 
    ...
    void getTokens(const string& input) {
        ...
        rep(i,0,len-1){
            ...
            if(c=='\"'||c=='\''){//Strings
                ...
                token[++toptk]=TOKEN{"String",mpc,word,rex[tmpi],rey[tmpi]};
                continue;
            }
            if(isdigit(c)){//Numbers
                ...
                i--;
                token[++toptk]=TOKEN{"Number",bsnum,word,rex[tmpi],rey[tmpi]};
                continue;
            ...
            if(isStValidChar(c)){//Keywords or Identifiers
                ...
                if(mp.find(word)!=mp.end()){//Keywords
                    token[++toptk]=TOKEN{"Keyword",mp[word],word,rex[tmpi],rey[tmpi]};
                }
                else token[++toptk]=TOKEN{"Identifier",bsid,word,rex[tmpi],rey[tmpi]};//Identifiers
                continue;
            }
            if(bsop<mpc&&mpc<=bshaku){//Operators or delimiters
                ...
                while(--oplen&&!flag) if(mp.find(word=input.substr(i,oplen))!=mp.end()){//find from long
                    ...
                    flag=1;
                    token[++toptk]=TOKEN{(mpv<=bsdeli)?("Operators"):("Delimiter"),mpv,word,rex[tmpi],rey[tmpi]};
                    i+=oplen-1;
                }  
...
public:
    ...
    void work(string filein){
        ...
        getTokens(codelow);
        token[0].word=token[toptk+1].word=token[toptk+2].word="#";// boundary case
    ...
    void printTokens(string fileout){
        ...
        fout << std::left;
        rep(i,1,toptk) {
            fout << "(" << setw(3) << token[i].sym << " , " << setw(25) << token[i].word << ")"
                << "         Type: " << setw(10) << token[i].type << "  idx: " << setw(4) << token[i].idx << "  idy: " 
                << setw(4) << token[i].idy << endl;
    ...
    //check kind    Need to be adjusted according to string keywords[DI_SZ] in lexer
    // bool isDelare(TOKEN tmp){
    //     return 1<=tmp.sym&&tmp.sym<=18;
    // }

    bool isStorageClassSpecifiers(TOKEN tmp){
        return 0<=tmp.sym&&tmp.sym<=4;
    }
    bool isTypeSpecifiers(TOKEN tmp){
        return 5<=tmp.sym&&tmp.sym<=14;
    }
    bool isTypeQualifiers(TOKEN tmp){
        return 15<=tmp.sym&&tmp.sym<=17;
    }
    bool isControlStatements(TOKEN tmp){
        return 18<=tmp.sym&&tmp.sym<=29;
    }
    bool isOperators(TOKEN tmp){
        return 30<=tmp.sym&&tmp.sym<=39;
    }
    bool isExceptionHandling(TOKEN tmp){
        return 40<=tmp.sym&&tmp.sym<=42;
    }
    bool isClassesAndObjects(TOKEN tmp){
        return 43<=tmp.sym&&tmp.sym<=49;
    }




    bool checkOPLevel(TOKEN tmp,int rank){
        return getOPLevel(tmp)==rank;
    }
    int getOPLR(int v){// 1: L->R  2: R->L -1:-1
        if(v==0) return -1;
        if(
            v==2||v==14||v==15
        ) return 2;
    ...
    int getOPLevel(TOKEN tmp){
        if(isOPLevel1(tmp)) return 1;
        if(isOPLevel2(tmp)) return 2;
        if(isOPLevel3(tmp)) return 3;
        if(isOPLevel4(tmp)) return 4;
        if(isOPLevel5(tmp)) return 5;
        if(isOPLevel6(tmp)) return 6;
        if(isOPLevel7(tmp)) return 7;
        if(isOPLevel8(tmp)) return 8;
        if(isOPLevel9(tmp)) return 9;
        if(isOPLevel10(tmp)) return 10;
        if(isOPLevel11(tmp)) return 11;
        if(isOPLevel12(tmp)) return 12;
        if(isOPLevel13(tmp)) return 13;
        if(isOPLevel14(tmp)) return 14;
        if(isOPLevel15(tmp)) return 15;
        if(isOPLevel16(tmp)) return 16;
        return 0;
    }
    bool isOPLevel1(TOKEN tmp){ //L->R
        if(
            tmp.word == "()" || // need to do
            tmp.word == "[]" || // need to do
            tmp.word == "->" || 
    ...
    bool isOPLevel9(TOKEN tmp){ //L->R
        if(
            tmp.word == "&"
        ) return 1;
        return 0;
    }
    bool isOPLevel10(TOKEN tmp){ //L->R
        if(
            tmp.word == "^"
        ) return 1;
        return 0;
    }
    bool isOPLevel11(TOKEN tmp){ //L->R
        if(
            tmp.word == "|"
        ) return 1;
        return 0;
    }
    bool isOPLevel12(TOKEN tmp){ //L->R
        if(
            tmp.word == "&&"
        ) return 1;
        return 0;
    }
    bool isOPLevel13(TOKEN tmp){ //L->R
        if(
            tmp.word == "||"
        ) return 1;
        return 0;
    }
    bool isOPLevel14(TOKEN tmp){ //R->L // c?a:b
        if(
            tmp.word == "?"
        ) return 1;
        return 0;
    }
    bool isOPLevel15(TOKEN tmp){ //R->L
        if(
            tmp.word == "=" ||
            tmp.word == "+=" ||
            tmp.word == "-=" ||
    ...
    bool isOPLevel16(TOKEN tmp){ //L->R
        if(
            tmp.word == ","
        ) return 1;
        return 0;
